...
// Component tự zoom đến vị trí hiện tại
...
function DestinationPicker({ onPick }: { onPick: (pos: LatLngTuple) => void }) {
  useMapEvents({
...
// Bật tất cả interaction của map (drag, zoom,…)
...
export default function LeafletMap() {
  ...
  // PATCH nhỏ cho lỗi "Map container is already initialized" (dev mode)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-2 pointer-events-auto">
        ...
        {status && (
          <div className="mt-1 rounded-md bg-white/90 px-2 py-1 text-xs shadow pointer-events-none">
            {status}
            {accuracy !== null && (
        ...
        {routeInfo && (
          <div className="mt-1 rounded-md bg-white/90 px-2 py-1 text-xs shadow pointer-events-none">
            Quãng đường: {(routeInfo.distance / 1000).toFixed(2)} km
            <br />
      ...
      <MapContainer
        center={position ?? [10.0452, 105.7469]}
        zoom={position ? 16 : 13}
